...
export function OverviewStats({
  ...
  const stats = [
    ...
    },
    {
      ...
      icon: ShieldCheck,
      color:
        kycStatus === "APPROVED" ? "text-emerald-600" : "text-amber-500",
      bg:
        kycStatus === "APPROVED" ? "bg-emerald-50" : "bg-amber-50",
      pulse: true,
    },
  ];
        ...
        return (
          ...
          >
            <div className="flex items-center justify-between mb-6">
              ...
              <div
                className={`w-12 h-12 rounded-2xl ${stat.bg} flex items-center justify-center ${stat.color} border border-transparent group-hover:border-current transition-all`}
              >
            ...
            <div className="flex items-center gap-3">
              ...
              {stat.pulse && !isLoading && (
                <div
                  className={`w-2 h-2 rounded-full animate-pulse ${stat.color.replace("text", "bg")}`}
                />
              )}
            </div>
            ...
            <div className="mt-4 h-1 w-full bg-slate-50 rounded-full overflow-hidden">
              <div
                className={`h-full ${stat.color.replace("text", "bg")} w-2/3 opacity-20`}
              />
            </div>
          </div>
        );
